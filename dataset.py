...
def create_smote_dataset(x, y, filename, path):
    """
    Generate and save SMOTE-augmented datasets with varying sampling strategies.

    This function generates SMOTE (Synthetic Minority Over-sampling Technique) augmented datasets
    with different sampling strategies and saves them to a specified file using joblib.

    Parameters:
        x (numpy array): Input features for training.
        y (numpy array): Target labels for training.
        filename (str): The filename for the saved datasets.
        path (str): The directory path where the datasets will be saved.

    """
    datasets = {}
    x1, y1 = select_set(x, y, 1)
    x0, y0 = select_set(x, y, 0)
    ratio = np.size(x1) / np.size(x0)
    for i in range(1, 6):
        sampling_strategy = i * 0.1
        if ratio < sampling_strategy:
            smote = SMOTE(sampling_strategy=sampling_strategy, random_state=42)
            x_train_aug, y_train_aug = smote.fit_resample(x, y)
            datasets[f"TrainAug_{sampling_strategy}"] = (x_train_aug, y_train_aug)
