...
# Create subdirectories 'sample', 'smote', 'samplesmote' and 'unet'
subdirectories = ['sample', 'smote', 'samplesmote', 'unet']
...
# Define model names and paths for Sample+SMOTE combined models
model_name_smote = "samplesmote_model_"
path = 'output/samplesmote/'
# Create Sample+SMOTE combined datasets and perform Grid Search for each model
create_samplesmote_dataset(trainX, trainY, model_name_smote, path, n_values=range(2, 11))
sample_grindseach_learn(param_grid, model_name_smote, path, start=2)
# Test Sample+SMOTE combined models
sample_test(model_name_smote, trainX, trainY, testX, testY, path)
